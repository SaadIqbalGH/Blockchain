...
contract CappedLockableToken is ERC20, Ownable {
    ...
    event tokenPending(string info, uint tokens_num );
    
    constructor() ERC20("Cap Lock Token", "CLT") {
        initialSupply = 500000 * (10**uint(decimals()));
        _cap = initialSupply.mul(20000);
        releaseTime = block.timestamp + 30 days;
        _Owner = msg.sender;
        _mint(msg.sender, initialSupply); 
        _balances[_Owner] = totalSupply();
        Token_Sale_Lot = 100 * (10**uint(decimals()));
        Lot_Sell_Price = 1000000000000000000  wei;
    }
    
    function generateToken(address account, uint _Tokens) public onlyOwner {
